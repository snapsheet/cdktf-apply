...
describe('validate pipeline (normalize → extract → compare)', () => {
  it('reports plans as matching when only JSON key order differs in string fields', () => {
    ...
    const containerDefsOrder2 =
      '[{"logConfiguration":{"options":{"awslogs-region":"us-east-1","awslogs-group":"x"}},"name":"web"}]';
    // Fix the second one - it was malformed. Same content, different key order:
    const containerDefsOrder2Fixed =
    ...
    const newPlanRaw = makePlanJson(containerDefsOrder2Fixed);
  ...
  it('reports plans as differing when actual values differ', () => {
    const oldPlanRaw = makePlanJson(
      '[{"name":"web","image":"img:1"}]'
    );
    const newPlanRaw = makePlanJson(
